...
def test_recall_does_not_create_store_file_when_none_exists(tmp_path):
    ...
    assert not (tmp_path / ".urdyn" / "memory.db").exists()
...
def test_sqlite_error_during_read_is_translated_to_urdyn_error(tmp_path):
    ...
    UrdynStorageError, not a raw sqlite3 exception."""
    cx = Urdyn.init(tmp_path, "dev")
    cx.remember("a memory")
