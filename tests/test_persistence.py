...
These tests must exercise real persistence: a fresh `Urdyn` object
...
def test_memory_survives_discovery_from_fresh_instance(tmp_path):
    ...
    rediscovered = Urdyn.discover(tmp_path)
...
def test_two_workspaces_do_not_share_memory(tmp_path):
    ...
    cx_a = Urdyn.init(workspace_a, "dev")
    cx_b = Urdyn.init(workspace_b, "dev")
