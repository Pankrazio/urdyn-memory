...
does Cortex know" — a conclusion, stated as a sentence. A `Skill` answers a
...
an existing Lesson (see `Cortex.promote`), which is where its
...
class Skill:
    """A single recorded procedure, persisted by Cortex.
    ...
    why Cortex considers this procedure valid. `evidence_ids` is copied
    ...
    even though the record of why Cortex once considered it valid never
    does. A5 does not implement Skill revalidation; any future change in
    validity must be represented through new Cortex history/state, not
